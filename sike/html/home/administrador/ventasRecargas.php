...
$query_select2 = mysqli_query($con, "SELECT v.idventarecarga,v.fecha,v.status,v.totalventa,v.descripcion,v.factura,u.nombre_usuario,CONCAT(s.numero,' ',s.direccion) AS sucursal,
v.numero, e.nombre AS empresa FROM ventarecarga v
INNER JOIN usuarios u ON v.usuarios_idusuarios = u.id
INNER JOIN sucursal s ON v.sucursal_idsucursal = s.id 
INNER JOIN empresa e ON e.id = v.empresa 	WHERE s.id = $Intid");
...
<table class="table  table-condensed table-hover table-responsive-md  justify-center " id="tablaProductos">
  ...
  <tbody>
    ...
    ?>
        ...
        <td class="text-center"><?php echo $row2['idventarecarga'] ?></td>
          <td><?php echo $row2['fecha'] ?></td>
          <td><?php echo $row2['status'] ?></td>
          <td><?php echo $row2['totalventa'] ?> </td>
          <td><?php echo $row2['descripcion'] ?> </td>
          <td><?php echo $row2['factura'] ?> </td>
          <td><?php echo $row2['nombre_usuario'] ?> </td>
