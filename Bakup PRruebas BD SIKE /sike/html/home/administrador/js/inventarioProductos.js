...
$(document).ready(function(){
  ...
  "columnDefs":[{
    ...
    "defaultContent": "<div class='text-center'><div class='btn-group'> <button type='button' class='btn btn-success btnVerProductoInventario'><i class='far fa-eye'></i></button> <button class='btn btn-warning btnEditarProductoInventario'>Editar<i class='far fa-edit'></i></button>  </div></div>"  
   }],
...
"language": {
 ...
 //incializa la tablaAsignarProveedores con la libreria DataTable que lista todos los proveedores
 tablaRecargaInventario = $('#tablaRecargaInventario').DataTable({  // incializa la tabla proveedores
...
//  $('#tablaProductosSinAsignar').DataTable().destroy();//destruye el datatable en caso de haberse incializado antes
 //  var tablaProductosSinAsignar = $('#tablaProductosSinAsignar').DataTable({
   ...
   $.ajax({//aqui se indica que vamos a hacer con los datos obtenidos del formulario
          type: "POST",
          url: "./ajax/ajaxInventarioProductos.php", //indica el Ajax donde se procesara los parametros enviados 
              ...
              if(arregloProductosSinAsignar == 'replica'){ // en caso de que el valor de data2 que viene del ajax sea replica es porque la comparacion con BD ya existia el dato indica que no hay prductos sin asociar
                  ...
                  var tablaProductosSinAsignar = $("#tablaProductosSinAsignar").DataTable({
                      "ajax":{
                          ...
                          "url":"./ajax/ajaxInventarioProductos.php"
                  ...
                  console.log("entrando cargar productos para asignar");
                  $('#tablaRecargaInventario').DataTable().destroy();//destruye el datatable en caso de haberse incializado antes
                  var tablaRecargaInventario = $("#tablaRecargaInventario").DataTable({
                      "ajax":{
                          "method":"POST",
                          "data": {action:"cargarRecargaInventario",idSucursal:idSucursal},
                          "url":"./ajax/ajaxInventarioProductos.php"
                      },//din del ajax dentro de otro DataTable
                      "columns":[
                          {"data":"nombre"},
...
// --------------FIN DEL SELECT SeleccionSucursalAsignar
  ...
  $(document).on("click", ".btnVerProductoInventario", function(){
  ...
  $('#categoriaProductoInventario').prop('disabled', true);
  $('#existenciaProductoInventario').prop('disabled', true);
  $('#precioCostoProductoInventario').prop('disabled', true);
  $('#precioVentaProductoInventario').prop('disabled', true);
  $('#precioMinimoProductoInventario').prop('disabled', true);
  $('#precioPromocionProductoInventario').prop('disabled', true);
  
  
  const $estadoBTNGuardar = document.querySelector("#btnGuardarProductoInventario");//selecciona el elemento del modal y lo pasa a una variable local
  $estadoBTNGuardar.style.display = "none"; // oculta el boton guardar
  
  
  idSucursal=$("#SeleccionSucursalAsignar").val();//obitne el value del option select que estamos seleccionamos
  ...
  $.ajax({ //ajax que va obtener valores de tabla de proveedor con id
      url: './ajax/ajaxInventarioProductos.php', //al documento php ajax al cual iran los datos y de donde retornara valores de la consulta
      type: "POST",
      //dataType: 'json', 
      async: true,
      data: {action:action,intencion:intencion,idProductoSinAsignar:idProductoSinAsignar,idSucursal:idSucursal}, //envia valores al ajax action y el id
        ...
        console.log(action);
          ...
          if (response != 'error') {
            ...
            //  data2 = JSON.parse(data2);
              $('#codigoProductoSinAsignar').val(data3.idProducto); // carga el valor de data2.nombre en un input del modal nuevo_proveedor el cual tenga el id nombreComercial y los de abajo tambien
              ...
              $('#categoriaProductoInventario').val(data3.nombreCategoria);
              $('#subcategoriaProductoSinAsignar').val(data3.nombreSubCategoria);
              $('#idPrecioProductoInventario').val(data3.idprecios);
              $('#existenciaProductoInventario').val(data3.existencia);
              $('#precioCostoProductoInventario').val(data3.precio_costo);
              $('#precioVentaProductoInventario').val(data3.precio_venta);
              $('#precioMinimoProductoInventario').val(data3.precio_minimo);
              $('#precioPromocionProductoInventario').val(data3.precio_promocion);
              $('#estadoProductoInventario').val(data3.estado);
              ...
              $(".modal-title").text("Informacion del Producto");//titulo del header
  ...
  $(document).on("click", ".btnEditarProductoInventario", function(){
    // estado de inputs o botons habilita o deshabilita
    $('#codigoProductoSinAsignar').prop('disabled', true);
    $('#skuProductoSinAsignar').prop('disabled', true);
    $('#nombreProductoSinAsignar').prop('disabled', true);
    ...
    $('#existenciaProductoInventario').prop('disabled', true);
    $('#precioCostoProductoInventario').prop('disabled', false);
    $('#precioVentaProductoInventario').prop('disabled', false);
    $('#precioMinimoProductoInventario').prop('disabled', false);
    $('#precioPromocionProductoInventario').prop('disabled', false);

    const $estadoBTNGuardar = document.querySelector("#btnGuardarProductoInventario");//selecciona el elemento del modal y lo pasa a una variable local
    $estadoBTNGuardar.style.display = "Block"; // oculta el boton guardar
    idSucursal=$("#SeleccionSucursalAsignar").val();//obitne el value del option select que estamos seleccionamos
    fila = $(this).closest("tr");
    idProductoSinAsignar = parseInt(fila.find('td:eq(0)').text());
    codigo = fila.find('td:eq(1)').text();
          ...
          console.log(action);
            ...
            if (response != 'error') {
              ...
              //  data2 = JSON.parse(data2);
                ...
                $(".modal-header").css("color","white"); //cambia el color de texto del header a blanco 
                $(".modal-title").text("Editar Precio del Producto");//titulo del header
                $("#modalVerProductoSinAsignar").modal("show"); //al clickear el boton nuevo proveedor lanza el modal que tiene el id Modal_Nuevo_Proveedor el cual es una clase alojada en /modal/editarProveedor llamada desde el archivo verProveedor
  ...
  $(document).on("click", ".btnEditarProductoInventarioPRUBA", function(){
  // estado de inputs o botons habilita o deshabilita
  ...
  $("#formVerProductoSinAsignar").submit(function( event ) {
  // estado de inputs o botons habilita o deshabilita
  idProductoAsignar=$.trim($("#codigoProductoSinAsignar2").val());
  
  idPrecioProductoInventario=$.trim($("#idPrecioProductoInventario").val());
  existencias=$.trim($("#existenciaProductoInventario").val());
  precioCosto=$.trim($("#precioCostoProductoInventario").val());
  precioVenta=$.trim($("#precioVentaProductoInventario").val());
  precioMinimo=$.trim($("#precioMinimoProductoInventario").val());
  precioPromocion=$.trim($("#precioPromocionProductoInventario").val());
  estadoProducto=$('select[id="estadoProductoInventario"] option:selected').text();
  ...
  //al presionar el boton nuevo la variable global action cambia a agregar producto 
         ...
         $.ajax({ //ajax que va obtener valores de tabla de proveedor con id
            ...
            data: {action:action,intencion:intencion,
              ...
              idPrecioProductoInventario:idPrecioProductoInventario,
              idSucursal:idSucursal   }, //envia valores al ajax action y el id
            success: function(data2){ //recibe una respuesta con una array json
              ...
              if(data2 == 'replica'){ // en caso de que el valor de data2 que viene del ajaxProveedore sea replica es porque la comparacion con BD ya existia el dato y no se pudo ejecutar la consulta 
                  Swal.fire({
                  title: "ERROR al Actualizar PRECIO", //titulo del modal
                  icon: 'error', //tipo de advertencia modal
                  timer: 3000                     
                  });
                   // vacia el input ProveedorNIT del modal 
                  console.log("rechazo Actualizacion PRECIO ERROR");   // // imprime en consola para el desarrolador ver el valro que esta obteniendo 
                }
                ...
                Swal.fire({
                        ...
                        timer: 2000
                          ...
                          window.location = "inventarioProductos.php";
                        });
